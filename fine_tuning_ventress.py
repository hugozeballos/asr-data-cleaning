...
# Modificar `prepare_dataset` para mostrar progreso en `map()`
...
#    import os
    ...
    # Verificar si torch.distributed ya está inicializado
#    if not dist.is_initialized():
#        dist.init_process_group(backend="nccl")

#    print(f"🔹 RANK: {os.environ.get('RANK')}, WORLD_SIZE: {os.environ.get('WORLD_SIZE')}")
#    print(f"🔹 Usando GPUs: {os.environ.get('CUDA_VISIBLE_DEVICES')}")
...
#    model, train_dataset, val_dataset = accelerator.prepare(model, train_dataset, val_dataset)
    ...
    for iteration in tqdm(range(num_iterations), desc="🔄 Iteraciones de entrenamiento"):
        ...
        print("Infiriendo...")
        train_dataset = train_dataset.map(lambda batch: infer_batch(batch, processor, model), batched=True, batch_size=16, num_proc=1, desc="Inferencia en batch")
        ...
        train_dataset = train_dataset.map(compute_cer_sample, num_proc=4, desc="Calculando CER")
        ...
        log_file = "removed_ids_small.json"
        try:
            with open(log_file, "r") as f:
